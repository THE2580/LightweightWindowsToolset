...
- 最新回归修复：打包/构建模式下计时器独立窗口 `loadFile` hash 已改为 `#/...` 路由格式，修复本地时间窗口打不开；验证方式为 Playwright Electron 启动构建产物、点击本地时间卡片、确认出现 `#/timer-clock` 窗口且正文包含“本地时间”。
- 当前准备发布 `1.2.6`：版本号和 README 已更新，用于替换 `1.2.5` 中计时器独立窗口路由回归。
